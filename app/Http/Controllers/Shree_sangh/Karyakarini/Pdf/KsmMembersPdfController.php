...
use App\Models\ShreeSangh\Karyakarini\Pdf\KsmMembersPdf;
use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Facades\Validator;

class KsmMembersPdfController extends Controller
{
    public function index()
    {
        $pdfs = KsmMembersPdf::orderBy('session', 'desc')->orderBy('created_at', 'desc')->get();
    ...
    public function store(Request $request)
    {
        ...
        $pdf = KsmMembersPdf::create($data);
        return response()->json(['message' => 'PDF successfully uploaded!', 'data' => $pdf], 201);
    }

    public function show(string $id)
    {
        return response()->json(KsmMembersPdf::findOrFail($id));
    }

    public function update(Request $request, string $id)
    {
        $pdf = KsmMembersPdf::findOrFail($id);
