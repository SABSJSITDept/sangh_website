...
@section('content')
    ...
    <body>
        <div class="container py-5">
            <div class="page-header mb-4 d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <div class="small-muted">Add, edit, delete results and allow students to fetch their JSP result card.
                    </div>
                </div>
                <div class="text-end">
            ...
            <div class="card card-rounded p-3 mb-4 bg-white border-0 shadow-sm">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div>
                        ...
                        <p class="text-muted small mb-0">Control whether students can search and view their results on the
                            frontend.</p>
                    </div>
                    <div class="form-check form-switch form-switch-md d-flex align-items-center">
                        <input class="form-check-input" type="checkbox" role="switch" id="resultVisibilityToggle"
                            style="width: 2.8rem; height: 1.5rem; cursor: pointer;">
                        <label class="form-check-label fw-semibold ms-2" for="resultVisibilityToggle"
                            id="visibilityStatusText">Loading...</label>
                    </div>
                </div>
            </div>
            ...
            <div class="card card-rounded p-3">
                <ul class="nav nav-tabs mb-3" id="jspTabs" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="add-data-tab" data-bs-toggle="tab" data-bs-target="#add-data"
                            type="button" role="tab" aria-controls="add-data" aria-selected="true">Add / Edit Data</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="fetch-result-tab" data-bs-toggle="tab" data-bs-target="#fetch-result"
                            type="button" role="tab" aria-controls="fetch-result" aria-selected="false">Fetch
                            Result</button>
                    </li>
                </ul>
                    ...
                    <div class="tab-pane fade show active" id="add-data" role="tabpanel" aria-labelledby="add-data-tab">
                        <form id="jspResultForm" class="row g-3 mb-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="tel" name="Mobile" class="form-control" placeholder="10-digit mobile"
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-text small-muted">You can enter a number or characters (e.g., 1, 2, A+,
                                    Topper, etc.).</div>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-12 d-flex gap-2">
                                ...
                                <div class="ms-auto small-muted align-self-center">Tip: Click a row in the table below to
                                    edit.</div>
                            </div>
                        </form>
                    ...
                    <div class="tab-pane fade" id="fetch-result" role="tabpanel" aria-labelledby="fetch-result-tab">
                        <div class="card p-3 card-rounded">
                            <div id="fetchVisibilityWarning" class="alert alert-warning d-none mb-3">
                                ⚠️ <strong>Note:</strong> Results are currently set to <strong>Hidden</strong>. Students
                                won't be able to search or view their results on the frontend.
                            </div>
                            <form id="fetchResultForm" class="row g-3">
                                ...
                                <div class="col-md-4 d-flex align-items-end">
                                    <button type="button" class="btn btn-primary w-100" onclick="fetchResult()">Find
                                        Result</button>
                                </div>
                            </form>
        ...
        <script>
            ...
            function renderResultCardHtml(row) {
                return `
            ...
            async function fetchResult() {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (rows.length === 1) {
                            // Single student: Show result card immediately
                            const card = document.createElement('div');
                            card.className = 'card card-rounded p-4 mb-3';
                            card.style.backgroundColor = '#ffffff';
                            card.innerHTML = renderResultCardHtml(rows[0]);
                            resultDiv.appendChild(card);
                        } else {
                            // Multiple students: Show selection option
                            const selectContainer = document.createElement('div');
                            selectContainer.className = 'card card-rounded p-4 mb-3 border-primary';
                            selectContainer.style.backgroundColor = '#ffffff';
                            
                            let optionsHtml = '<option value="">-- Choose Student --</option>';
                            rows.forEach(row => {
                                const guardianText = row.Guardian_Name ? ` (Guardian: ${row.Guardian_Name})` : '';
                                const cityText = row.City ? `, ${row.City}` : '';
                                optionsHtml += `<option value="${row.id}">${row.Student_Name}${guardianText}${cityText}</option>`;
                            });

                            selectContainer.innerHTML = `
                                <h5 class="fw-bold mb-2 text-primary">
                                    <i class="bi bi-people-fill me-2"></i>Multiple Results Found
                                </h5>
                                <p class="text-muted small mb-3">
                                    There are multiple students registered with this mobile number. Please select the student to view their result card:
                                </p>
                                <div class="row g-3 align-items-center">
                                    <div class="col-md-8">
                                        <select id="studentSelect" class="form-select border-primary" onchange="showSelectedStudentResult(this.value)">
                                            ${optionsHtml}
                                        </select>
                                    </div>
                                    <div class="col-md-4">
                                        <button type="button" class="btn btn-outline-primary w-100" id="resetSelectionBtn" onclick="resetStudentSelection()">Clear Selection</button>
                                    </div>
                                </div>
                                <div id="selectedResultContainer" class="mt-4"></div>
                            `;
                            resultDiv.appendChild(selectContainer);
                        }
                    } else {
                        const err = await response.json(); 
                        showToast(err.message || 'Error fetching', 'danger');
                    }
                } catch (err) { 
                    console.error(err); 
            ...
            function showSelectedStudentResult(id) {
                const container = document.getElementById('selectedResultContainer');
                container.innerHTML = '';
                if (!id) return;
                
                const row = fetchedResults.find(r => r.id == id);
                if (row) {
                    const card = document.createElement('div');
                    card.className = 'card card-rounded p-4 mb-0 border-0 shadow-sm';
                    card.style.backgroundColor = '#ffffff';
                    card.innerHTML = renderResultCardHtml(row);
