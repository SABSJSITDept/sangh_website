...
@section('content')
    ...
    <div class="container mt-4">
        ...
        <ul class="nav nav-tabs mb-3" id="sessionTabs" role="tablist">
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="tab-2023-25" data-bs-toggle="tab" data-session="2023-25" type="button">
                    2023-25
                </button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="tab-2025-27" data-bs-toggle="tab" data-session="2025-27" type="button">
                    2025-27
                </button>
            </li>
        </ul>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            let currentSession = '2025-27'; // Default session
            ...
            const fetchList = (session = currentSession) => {
                fetch('/api/it-cell')
                    .then(res => res.json())
                    .then(data => {
                        list.innerHTML = '';
                        // Filter by session
                        const filtered = data.filter(item => item.session === session);
                        ...
                        filtered.forEach(item => {
                            ...
                            row.innerHTML = `
                                ...
                                <td>${item.mobile}</td>
                                <td>${item.priority}</td>
                                <td><span class="badge bg-info">${item.session}</span></td>
                                <td>
            ...
            // Tab switching
            document.querySelectorAll('#sessionTabs button').forEach(tab => {
                tab.addEventListener('click', function () {
                    document.querySelectorAll('#sessionTabs button').forEach(t => t.classList.remove('active'));
                    this.classList.add('active');
                    currentSession = this.getAttribute('data-session');
                    fetchList(currentSession);
                });
            });
                ...
                fetch(`/api/it-cell`)
                    ...
                    .then(data => {
                        ...
                        if (item) {
                            ...
                            form.session.value = item.session;
