...
@section('content')
            ...
            <div class="col-md-5 mb-4">
                <div class="card shadow-lg border-0 rounded-4"
                    ...
                    <div class="card-header text-white fw-bold border-0 py-3">
                        <h5 class="mb-0">
                            <i class="fas fa-file-pdf me-2"></i>📄 प्रवृत्ति संयोजक PDF अपलोड करें
                        </h5>
                    </div>
                    <div class="card-body bg-white rounded-bottom-4">
                        <form id="pravartiSanyojakPdfForm" enctype="multipart/form-data">
                            @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Form Submit Handler
            document.getElementById('pravartiSanyojakPdfForm').addEventListener('submit', async function (e) {
                e.preventDefault();
                const form = e.target;
                const formData = new FormData(form);
                const id = document.getElementById('editId').value;
                const url = id ? `/api/pravarti-sanyojak-pdf/${id}` : '/api/pravarti-sanyojak-pdf';

                if (id) {
                    formData.append('_method', 'PUT');
                }

                // Validate PDF is required for new entries
        ...
        function fetchData() {
            fetch('/api/pravarti-sanyojak-pdf')
                .then(res => res.json())
                .then(data => {
                    ...
                    if (data.length === 0) {
                        tbody.innerHTML = `
                                                        <tr>
                                                            <td colspan="4" class="text-center text-muted py-4">
                                                                <i class="fas fa-inbox"></i> कोई PDF उपलब्ध नहीं है।
                                                            </td>
                                                        </tr>
                                                    `;
                        return;
                        ...
                        tbody.innerHTML += `
                                                        <tr>
                                                            ...
                                                            <td class="text-center">
                                                                <span class="badge bg-info">${item.session}</span>
                                                            </td>
                                                            <td class="text-center">
                                                                <div class="d-flex gap-1 justify-content-center flex-wrap">
                                                                    <a href="/storage/${item.pdf}" target="_blank" 
                                                                        class="btn btn-success btn-sm d-flex align-items-center gap-1" 
                                                                        title="PDF देखें">
                                                                        <i class="fas fa-eye"></i>
                                                                        <span class="d-none d-md-inline">देखें</span>
                                                                    </a>
                                                                    <button onclick="editPdf(${item.id})" 
                                                                        class="btn btn-warning btn-sm d-flex align-items-center gap-1" 
                                                                        title="संपादित करें">
                                                                        <i class="fas fa-edit"></i>
                                                                        <span class="d-none d-md-inline">संपादित</span>
                                                                    </button>
                                                                    <button onclick="deletePdf(${item.id})" 
                                                                        class="btn btn-danger btn-sm d-flex align-items-center gap-1" 
                                                                        title="हटाएं">
                                                                        <i class="fas fa-trash"></i>
                                                                        <span class="d-none d-md-inline">हटाएं</span>
                                                                    </button>
                                                                </div>
                                                            </td>
                                                        </tr>
        ...
        // Edit PDF
        function editPdf(id) {
            fetch(`/api/pravarti-sanyojak-pdf/${id}`)
                .then(res => res.json())
                .then(data => {
        ...
        // Reset Form
        function resetForm() {
            document.getElementById('pravartiSanyojakPdfForm').reset();
